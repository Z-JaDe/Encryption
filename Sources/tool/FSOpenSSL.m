...
@implementation FSOpenSSL {
    BBRSACryptor* _rsaCryptor;
}

+ (NSString *)md5FromString:(NSString *)string {
    
    const char *cStr = [string cStringUsingEncoding:NSUTF8StringEncoding];
    unsigned long strLen = strlen(cStr);
    unsigned char result[MD5_DIGEST_LENGTH];
    NSMutableString *outStrg = [NSMutableString string];

    CC_MD5(cStr, (unsigned)strLen, result);
    unsigned int i;
...
+(NSString *)signString:(NSString *)kRSAPublicKey Text:(NSString *)text {
    FSOpenSSL* openssl = [[FSOpenSSL alloc] init];
    [openssl.rsaCryptor importRSAPublicKeyBase64:kRSAPublicKey];
    NSData *cipherData = [openssl.rsaCryptor encryptWithPublicKeyUsingPadding:RSA_PKCS1_PADDING plainData:[text dataUsingEncoding:NSUTF8StringEncoding]];
